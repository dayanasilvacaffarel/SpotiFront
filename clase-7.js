...
function mostrarDatosEnPerfil() {
    ...
    // let albumesTotales = document.getElementById("cant-albums");
    // if (albumesFamosos.length == 1) {
    //     albumesTotales.innerText = albumesFamosos.length + " album"
    // } else {
    //     albumesTotales.innerText = albumesFamosos.length + " albumes"
    // }
    // let albumesFavoritos = document.querySelectorAll("i");
    // console.log(albumesFavoritos);
    // let contadorfavoritos = 0;
    // if (albumesFamosos.like == true) {
    //     contadorfavoritos++;
    // }
    // console.log(contadorfavoritos);

    let cantAlbumes = albumesFamosos.length;
    let favoritos = 0;
    let spanAlbumes = document.getElementById('cant-albums');
    let spanFavoritos = document.getElementById('cant-favoritos');

    albumesFamosos.forEach(album =>album.like ? favoritos += 1 : undefined)

    cantAlbumes === 1 ? spanAlbumes.innerText = `${cantAlbumes} álbum` : spanAlbumes.innerText = `${cantAlbumes} álbumes`
    favoritos === 1 ? spanFavoritos.innerText = `${favoritos} favorito`: spanFavoritos.innerText = `${favoritos} favoritos`
